...
def annual_cate_development(data, development_args):
    ...
    for order, level in enumerate(development_args):
        input_data = data.loc[data['Reporter ISO'].isin(level)]

        fig, ax = plt.subplots(1, 4, figsize=(20, 5), sharey=True)
        ax_unpack = ax.ravel()
        fig.tight_layout()
        plt.subplots_adjust(wspace=0.1, hspace=0.3, top=0.85)
        fig.suptitle("Annual trade value of different categories for reporters in " + cat_p[order] +" development level", fontsize=15);
        ...
        reporter_category_value_data['Trade_Value'] = reporter_category_value_data['Trade_Value'] \
            .mask(reporter_category_value_data['Trade Flow'] == 'Import', -reporter_category_value_data['Trade_Value'])
        ...
        for count, reporter in enumerate(reporter_names):
            ...
            sns.barplot(x=report_import_data['Year'], y=report_import_data['Trade_Value'], hue='Category', data=report_import_data, ax=ax_unpack[count], alpha=0.5)
            ...
            sns.barplot(x=report_export_data['Year'], y=report_export_data['Trade_Value'], hue='Category', data=report_export_data, ax=ax_unpack[count])

            report_net_value = reporter_category_value_data.loc[reporter_category_value_data['Reporter ISO'] == reporter] \
                .groupby(['Year', 'Category']) \
                .agg(Net_Value=("Trade_Value", "sum")) \
                .reset_index() \
                .sort_values(by=['Year', 'Category'], ascending=True)
...
def reporter_category_pyramid(data, reporter_iso):
    ...
    for count, industrial_category in enumerate(cate_names):
        ...
        category_import_data = reporter_category_value_data.loc[reporter_category_value_data['Category'] == industrial_category] \
            .loc[reporter_category_value_data['Trade Flow'] == 'Import']
        sns.barplot(y=category_import_data['Year'], x=category_import_data['Trade_Value'], data=category_import_data, ax=ax_unpack[count], alpha=0.5)
            
        category_export_data = reporter_category_value_data.loc[reporter_category_value_data['Category'] == industrial_category] \
            .loc[reporter_category_value_data['Trade Flow'] == 'Export']
        sns.barplot(y=category_export_data['Year'], x=category_export_data['Trade_Value'], data=category_export_data, ax=ax_unpack[count])

        ax_unpack[count].set_title('Trade value of  ' + industrial_category, fontsize=15)
        # ax_unpack[count].grid(color='k', linestyle='--', alpha=0.5);
        ax_unpack[count].xaxis.set_label_text("Import / Export trade value (US$)", fontsize=12)
...
def plot_pie_diagram(data, reporter, year):
    ...
    for count, y in enumerate(year):
        ...
        import_data_y = [list(z) for z in zip(data_y_import['Category'].unique().tolist(), data_y_import['Trade_Value'].unique().tolist())]
        ...
        export_data_y = [list(z) for z in zip(data_y_export['Category'].unique().tolist(), data_y_export['Trade_Value'].unique().tolist())]
...
def reporter_single_category_value(data, development_args, category_args):
    ...
    for _, level in enumerate(development_args):
        ...
        for count, reporter in enumerate(reporter_names):
            ...
            sns.barplot(x=report_data['Year'], y=report_data['Trade_Value'], hue='Trade Flow', data=report_data, ax=ax_unpack[count])
...
def development_category_value(data, development_args):
    ...
    fig.tight_layout()
    plt.subplots_adjust(wspace=0.1, hspace=0.3, top=0.85)
    fig.suptitle("Annual trade value of different categories for high development level", fontsize=15);
    ...
    for count, flow in enumerate(flow_direction):
        ...
        sns.lineplot(x=input_data['Year'], y=input_data['Trade_Value'], hue='Category', data=input_data, ax=ax_unpack[count], markers=True, style='Category')

        ax_unpack[count].grid(color='k', linestyle='--', alpha=0.5);
        ax_unpack[count].set_title(flow + " trade value (US$)", fontsize=12)
        ax_unpack[count].xaxis.set_tick_params(rotation=60)
        ax_unpack[count].yaxis.set_major_formatter(formatter)
...
def import_value_pattern(data_1, data_2, development_args):
    ...
    for count, data_input in enumerate(import_data):
        ...
        sns.lineplot(x=input_data['Year'], y=input_data['Trade_Value'], hue='Category', data=input_data, ax=ax[count], markers=True, style='Category')
...
def populated_value_category(data, populated_args, category_args):
    ...
    for count, country in enumerate(populated_args):
        ...
        sns.lineplot(x=input_data['Year'], y=input_data['Trade_Value'], hue='Trade Flow', data=input_data, ax=ax[count], markers=True, style='Trade Flow')
        ax[count].grid(color='k', linestyle='--', alpha=0.5);
...
def top_partner_category_value(data):
    ...
    partner_category_value_data['Trade_Value'] = partner_category_value_data['Trade_Value'] \
            .mask(partner_category_value_data['Trade Flow'] == 'Export', -partner_category_value_data['Trade_Value'])
    ...
    for count, partner in enumerate(top_partners):
        ...
        partner_import_data = partner_category_value_data.loc[partner_category_value_data['Partner ISO'] == partner] \
            ...
            .sort_values(by=['Year', 'Category'], ascending=True)
        sns.barplot(x=partner_import_data['Year'], y=partner_import_data['Trade_Value'], hue='Category', data=partner_import_data, ax=ax_unpack[count])
        ...
        partner_export_data = partner_category_value_data.loc[partner_category_value_data['Partner ISO'] == partner] \
            ...
            .sort_values(by=['Year', 'Category'], ascending=True)
        sns.barplot(x=partner_export_data['Year'], y=partner_export_data['Trade_Value'], hue='Category', data=partner_export_data, ax=ax_unpack[count], alpha=0.5)

        ax_unpack[count].grid(color='k', linestyle='--', alpha=0.5);
        ax_unpack[count].set_title('Trade value of different industrial product of ' + partner, fontsize=15)
        ax_unpack[count].legend(handles=[blue_patch, orange_patch, green_patch, red_patch], framealpha=0.7)
        
        ax_unpack[count].grid(color='k', linestyle='--');
        ax_unpack[count].xaxis.set_tick_params(rotation=60)
        ax_unpack[count].yaxis.set_major_formatter(formatter)
        ax_unpack[count].yaxis.set_label_text("Export / Import Value (US$)", fontsize=12)
        ax_unpack[count].set_yticklabels(['', '100B', '50B', '0','50B', '100B', '150B', '200B',  '' ])
...
def bottom_partner_category_value(data):
    ...
    for count, partner in enumerate(bottom_partners):
        ...
        partner_export_data = partner_category_value_data.loc[partner_category_value_data['Partner ISO'] == partner] \
            .loc[partner_category_value_data['Trade Flow'] == 'Export'] \
            .sort_values(by=['Year', 'Category'], ascending=True)
        sns.barplot(x=partner_export_data['Year'], y=partner_export_data['Trade_Value'], hue='Category', data=partner_export_data, ax=ax_unpack[count])
...
def reporter_top_partner_trade_value(data, developement_args):
    ...
    reporter_names = input_data['Reporter ISO'].unique()
    reporter_partner_year_value['Trade_Value'] = reporter_partner_year_value['Trade_Value'] \
        .mask(reporter_partner_year_value['Trade Flow']=='Export', -reporter_partner_year_value['Trade_Value'])
    ...
    for count, reporter in enumerate(reporter_names):
        ...
        sns.barplot(x=import_data['Year'], y=import_data['Trade_Value'], hue='Partner ISO', data=import_data, ax=ax_unpack[count])

        export_data = reporter_top_partner.loc[reporter_top_partner['Trade Flow']== 'Export']
        sns.barplot(x=export_data['Year'], y=export_data['Trade_Value'], hue='Partner ISO', data=export_data, ax=ax_unpack[count], alpha=0.8)

        ax_unpack[count].yaxis.set_major_formatter(formatter)
        ax_unpack[count].yaxis.set_label_text("Export / Import trade Value (US$)", fontsize=15)
        ax_unpack[count].xaxis.set_tick_params(rotation=60)
...
def reporter_partner_detail(input_data, reporter, partner):
    ...
    data_1['Trade_Value'] = data_1['Trade_Value'] \
        .mask(data_1['Trade Flow'] == 'Export', -data_1['Trade_Value'])

    import_data = data_1.loc[data_1['Trade Flow'] == 'Import']
    sns.barplot(x=import_data['Year'], y=import_data['Trade_Value'], data=import_data, hue='Category', ax=ax_unpack[0])

    export_data = data_1.loc[data_1['Trade Flow'] == 'Export']
    sns.barplot(x=export_data['Year'], y=export_data['Trade_Value'], data=export_data, hue='Category', ax=ax_unpack[0], alpha=0.5)

    ax_unpack[0].yaxis.set_major_formatter(formatter)
    ax_unpack[0].yaxis.set_label_text("Trade Value (US$)", fontsize=12)
    ...
    sns.lineplot(x=reporter_partner_data['Year'], y=reporter_partner_data['Trade Value (US$)'], data=reporter_partner_data, hue='Category', ax=ax_unpack[2])
    ax_unpack[2].yaxis.set_major_formatter(formatter)
    ax_unpack[2].yaxis.set_label_text("Trade Value (US$)", fontsize=12)
